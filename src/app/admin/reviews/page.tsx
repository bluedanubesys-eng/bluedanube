...
export default function Page() {
  ...
  useEffect(() => {
    erpGet("reviews", { shopId: CONFIG.defaultShopId }).then((res) => {
    ...
    <AdminLayout>
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-4xl font-black">Reviews</h1>
          <p className="mt-2 text-slate-500">Customer product reviews and moderation records.</p>
        </div>
      ...
      {loading ? (
        ...
        <DataTable title="Reviews" rows={rows} />
      )}
    </AdminLayout>
  );
}
